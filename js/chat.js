...
export async function loadMessages(roomId, limit = 100) {
  const { data, error } = await sb
    .from('messages')
    .select('id, room_id, author_key, content, created_at')
    .eq('room_id', roomId)
...
export async function sendMessage(roomId, content) {
  ...
  const author_key = sessionData?.session?.user?.id;
  const { data, error } = await sb
    .from('messages')
    .insert([{ room_id: roomId, content, author_key }])
    .select()
    .single();
  if (error) throw error;
  return data; // {id, room_id, content, author_key, created_at}
}
...
export function subscribeMessages(roomId, onInsert, channel) {
  ...
  chan.on(
    ...
    payload => {
      // payload.new = { id, room_id, author_key, content, created_at }
